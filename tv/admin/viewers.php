...
<section class="rounded-lg bg-white p-4 shadow-sm">
    <h3 class="text-base font-black">Current Viewer Counts</h3>
...
<section class="mt-4 rounded-lg bg-white p-4 shadow-sm">
    <h3 class="text-base font-black">Recent Viewer Sessions</h3>
    <div class="mt-4 overflow-x-auto">
        <table class="min-w-full text-left text-sm">
            <thead class="text-slate-500">
                <tr>
                    <th class="pb-2 pr-3">Event</th>
                    <th class="pb-2 pr-3">Viewer</th>
                    <th class="pb-2 pr-3">Started</th>
                    <th class="pb-2 pr-3">Last Seen</th>
            ...
            <tbody class="divide-y divide-slate-100">
                <?php foreach ($sessions as $session): ?>
                    <tr>
                        <td class="py-2 pr-3 font-semibold text-slate-800"><?= e($session['title']) ?></td>
                        <td class="py-2 pr-3 text-slate-600"><?= e(trim((string)(($session['first_name'] ?? '') . ' ' . ($session['last_name'] ?? ''))) ?: ($session['email'] ?: 'Guest')) ?></td>
                        <td class="py-2 pr-3 text-slate-600"><?= e(tv_format_datetime($session['started_at'])) ?></td>
                        <td class="py-2 pr-3 text-slate-600"><?= e(tv_format_datetime($session['last_seen_at'])) ?></td>
                        <td class="py-2 pr-3 text-slate-600"><?= e((string)$session['ip_address']) ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
