...
<div class="h-full flex flex-col">
    <div class="flex justify-between items-end mb-6 flex-shrink-0">
        <div>
            <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Customers</h2>
            <p class="text-sm text-slate-500 mt-1">Manage your business contacts and client profiles.</p>
        </div>
        
        <a href="<?= BASE_URL ?>/?page=customers&action=create" class="bg-[#1a1a1a] hover:bg-emerald-600 text-white p-3.5 rounded-2xl shadow-xl shadow-gray-200 transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center group" title="New Customer">
    ...
    <div class="flex-1 flex gap-6 min-h-0">
        <!-- Master List (Left) -->
        <div class="w-full md:w-80 lg:w-96 flex flex-col flex-shrink-0 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                ...
                <?php foreach ($customers as $customer): ?>
                    <a href="<?= BASE_URL ?>/?page=customers&id=<?= $customer['id'] ?>" class="flex items-center p-3 rounded-2xl transition-all group <?= $selectedId == $customer['id'] ? 'bg-emerald-50 border-emerald-100 ring-1 ring-emerald-500/20' : 'hover:bg-gray-50 border-transparent' ?> border">
                        <div class="w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center font-bold text-sm mr-3 transition-colors <?= $selectedId == $customer['id'] ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'bg-gray-100 text-gray-400 group-hover:bg-emerald-100 group-hover:text-emerald-600' ?>">
