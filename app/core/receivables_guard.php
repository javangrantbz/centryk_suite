...
/**
 * Shared gate for Receivables endpoints:
 *   - authenticated
 *   - POST with a JSON body carrying company_id
 *   - caller is an active admin/manager of that company
 *   - the company holds the 'receivables' entitlement ($writing => must be FULL)
 *
...
function receivables_guard(bool $writing): array
{
    ...
    Entitlements::assert($companyId, 'receivables', $writing);
