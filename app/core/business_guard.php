...
/**
 * Shared gate for Centryk Business module endpoints:
 *   - authenticated
 *   - POST with a JSON body carrying company_id
 *   - caller is an active admin/manager of that company
 *   - the company holds $packageKey ($writing => must be FULL, else READ is ok)
 *
 * @return array{0:int,1:int,2:array}  [userId, companyId, decodedBody]
 */
function business_guard(string $packageKey, bool $writing): array
{
    Auth::start();
    $user = Auth::user();
    if (!$user) {
    ...
    Entitlements::assert($companyId, $packageKey, $writing);
