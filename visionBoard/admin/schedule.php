...
<p class="text-slate-500 mb-6 text-sm max-w-2xl">
  A schedule tells the TV which playlist to show and when. Leave a field blank for “always”.
...
<div class="grid lg:grid-cols-3 gap-6">
  <div class="lg:col-span-1">
    <div class="bg-white rounded-xl shadow-sm p-6 sticky top-4">
      <h2 class="font-semibold mb-4"><?= $editing ? 'Edit schedule' : 'New schedule' ?></h2>
      <?php if (!$playlists): ?>
        <p class="text-sm text-slate-500">Create a <a class="text-green-700 underline" href="playlists.php">playlist</a> first.</p>
      <?php else: ?>
      <form method="post" class="space-y-3">
        ...
        <div>
          ...
          <select name="playlist_id" class="w-full rounded-lg border border-slate-300 px-3 py-2">
            <?php foreach ($playlists as $p): ?>
              <option value="<?= $p['id'] ?>" <?= ($editing['playlist_id'] ?? '')==$p['id']?'selected':'' ?>><?= e($p['name']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
          ...
          <div class="flex flex-wrap gap-1">
            <?php foreach ($DOW as $num=>$lbl): ?>
              <label class="cursor-pointer">
                ...
                <span class="inline-block px-2.5 py-1 rounded-lg border text-xs peer-checked:bg-green-700 peer-checked:text-white peer-checked:border-green-700"><?= $lbl ?></span>
              </label>
            <?php endforeach; ?>
          </div>
        </div>
        ...
        <div class="flex gap-2 pt-1">
          ...
          <?php if ($editing): ?><a href="schedule.php" class="px-4 py-2 rounded-lg border">Cancel</a><?php endif; ?>
        </div>
      </form>
      <?php endif; ?>
    </div>
  ...
  <div class="lg:col-span-2 space-y-6">
    <div class="bg-white rounded-xl shadow-sm p-4">
      <div class="flex items-center justify-between mb-3">
        <h2 class="font-semibold">Weekly calendar</h2>
          ...
          <div>
            ...
            <div class="relative h-96 rounded-lg bg-slate-50 border border-slate-200 overflow-hidden">
              ...
              <?php foreach ($calendarBlocks[$dayNum] as $block): ?>
                <div class="absolute left-1 right-1 rounded-md bg-green-700/90 text-white px-2 py-1 overflow-hidden shadow-sm"
    ...
    ?>
      <div class="bg-white rounded-xl shadow-sm p-4 <?= $s['is_enabled']?'':'opacity-60' ?>">
        <div class="flex items-start justify-between gap-3">
          <div>
            <p class="font-medium text-slate-800">🗓️ <?= e($s['name']) ?>
            ...
            <p class="text-sm text-slate-500 mt-1">▶ <?= e($s['playlist_name']) ?></p>
          ...
          <div class="flex items-center gap-3 shrink-0">
            <form method="post"><?= csrf_field() ?>
              ...
              <button class="text-xs text-slate-500 hover:underline"><?= $s['is_enabled']?'Disable':'Enable' ?></button>
            </form>
            <a href="schedule.php?edit=<?= $s['id'] ?>" class="text-sm text-green-700 hover:underline">Edit</a>
