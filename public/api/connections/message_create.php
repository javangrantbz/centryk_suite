...
if (!$member || ($member['role'] ?? '') !== 'admin') {
    Response::error('Only a company admin can send partner messages.', 403);
}

$result = ConnectionMessageService::create($companyId, $targetCompanyId, (int) $user['id'], $body);
if (empty($result['success'])) {
    Response::error((string) ($result['message'] ?? 'Could not send partner message.'));
}

Audit::log([
    'actor_user_id' => (int) $user['id'],
    'company_id' => $companyId,
    'event_type' => 'connection_message_created',
    'summary' => 'Sent a partner message',
    'metadata' => ['target_company_id' => $targetCompanyId],
]);

Response::ok(['message' => (string) ($result['message'] ?? 'Partner message sent.')]);
