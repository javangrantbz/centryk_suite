...
if (!$member || ($member['role'] ?? '') !== 'admin') {
    Response::error('Only a company admin can share campaigns.', 403);
}

$result = ConnectionCampaignShareService::create($companyId, $targetCompanyId, (int) $user['id'], $body);
if (empty($result['success'])) {
    Response::error((string) ($result['message'] ?? 'Could not share campaign.'));
}

Audit::log([
    'actor_user_id' => (int) $user['id'],
    'company_id' => $companyId,
    'event_type' => 'connection_campaign_shared',
    'summary' => 'Shared a campaign with a connected company',
    'metadata' => ['target_company_id' => $targetCompanyId],
]);

Response::ok(['message' => (string) ($result['message'] ?? 'Shared campaign sent.')]);
