...
$aiLogo   = trim((string)($_ENV['OPENAI_API_KEY'] ?? '')) !== ''; // AI logo button — off until a key is configured
                    ...
                    <div class="flex shrink-0 flex-col items-center min-[480px]:items-start gap-3">
                        ...
                        <div class="flex flex-col items-center min-[480px]:items-start gap-1.5">
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                <?php if ($aiLogo): ?>
                                <button type="button" id="aiLogoToggle" class="hidden items-center gap-1.5 rounded-xl border border-violet-400/30 bg-violet-500/15 px-4 py-2 text-xs font-black text-violet-200 transition hover:bg-violet-500/25">
                                    <i data-lucide="sparkles" class="h-3.5 w-3.5"></i> AI Logo
                                </button>
                                <?php endif; ?>
                            </div>
                            <p class="text-[9px] font-bold text-white/20 uppercase tracking-wider">PNG, JPG, WEBP or SVG · Max 2MB</p>
                            <?php if ($aiLogo): ?>
                            <div id="aiLogoPanel" class="mt-1 hidden w-full rounded-xl border border-violet-400/20 bg-violet-500/10 p-3">
                                <label class="block text-[9px] font-bold uppercase tracking-wider text-violet-200/70">Describe your logo (optional)</label>
                                <div class="mt-1 flex flex-wrap items-center gap-2">
                                    <input id="aiLogoPrompt" type="text" maxlength="300" placeholder="e.g. a friendly coffee cup, warm colours"
                                           class="min-w-[180px] flex-1 rounded-lg border border-violet-400/30 bg-white/6 px-3 py-2 text-xs text-white placeholder-white/25 outline-none focus:border-violet-400">
                                    <button type="button" id="aiLogoGenerate" class="shrink-0 rounded-lg bg-violet-600 px-3 py-2 text-xs font-bold text-white hover:bg-violet-500 disabled:bg-violet-400/50">Generate</button>
                                </div>
                                <div id="aiLogoActions" class="mt-2 hidden items-center gap-3">
                                    <button type="button" id="aiLogoUse" class="rounded-lg bg-violet-600 px-3 py-1.5 text-[11px] font-bold text-white hover:bg-violet-500">Use this logo</button>
                                    <button type="button" id="aiLogoRegen" class="text-[11px] font-semibold text-violet-200 hover:text-white">Regenerate</button>
                                    <button type="button" id="aiLogoCancel" class="text-[11px] font-semibold text-white/40 hover:text-white/70">Cancel</button>
                                </div>
                                <p id="aiLogoStatus" class="mt-2 text-[11px] text-violet-200/60">Generated by AI — review before using. You’re responsible for making sure it doesn’t copy an existing brand.</p>
                            </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
(function () {
    ...
    function setProfileEditable(canEdit) {
        ...
        var aiToggle = document.getElementById('aiLogoToggle');
        if (aiToggle) {
            aiToggle.classList.toggle('hidden', !canEdit);
            aiToggle.classList.toggle('flex', canEdit);
            if (!canEdit) { document.getElementById('aiLogoPanel').classList.add('hidden'); }
        }
    ...
    function loadProfile(companyId) {
        ...
        fetch('api/companies/profile.php?company_id=' + companyId)
            ...
            .then(function (data) {
                ...
                lastServerLogo = p.logo || '';
                setProfileLogo(lastServerLogo);
    ...
    // ── AI logo generation (toggle only rendered when OPENAI_API_KEY is set) ──
    (function () {
        var toggle = document.getElementById('aiLogoToggle');
        if (!toggle) { return; }
        var panel   = document.getElementById('aiLogoPanel');
        var genBtn  = document.getElementById('aiLogoGenerate');
        var actions = document.getElementById('aiLogoActions');
        var statusEl = document.getElementById('aiLogoStatus');
        var promptEl = document.getElementById('aiLogoPrompt');
        var defaultStatus = statusEl.textContent;
        var pendingDataUri = null;

        function dataUriToFile(uri, name) {
            var parts = uri.split(','), mime = parts[0].match(/data:([^;]+)/)[1];
        ...
        function generate() {
            ...
            fetch('api/companies/generate_logo.php', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ company_id: currentCompany.id, prompt: promptEl.value.trim() })
            })
                .then(function (r) { return r.json(); })
                .then(function (d) {
                    if (!d || !d.success) {
        ...
        document.getElementById('aiLogoUse').addEventListener('click', function () {
            if (!pendingDataUri) { return; }
            profileLogoFile = dataUriToFile(pendingDataUri, 'ai-logo.webp');
            reset();
        });
        document.getElementById('aiLogoCancel').addEventListener('click', function () {
            reset();
            if (profileLogoFile) {
                var reader = new FileReader();
                reader.onload = function (e) { setProfileLogo(e.target.result); };
                reader.readAsDataURL(profileLogoFile);
            } else {
                setProfileLogo(lastServerLogo);
            }
        });
