...
// Upcoming Belize public holidays — keeps the logged-out page feeling current.
// Today's holiday (if any) is called out; the rest of a ~10-day window is
// listed. If that window is empty we still show the single next holiday so the
...
$nextHoliday      = null;
try {
    foreach (PublicHolidays::forRange($today, date('Y-m-d', strtotime('+10 days'))) as $h) {
    ...
    if (!$holidayToday && !$upcomingHolidays) {
        $n = PublicHolidays::upcoming(1);
        $nextHoliday = $n[0] ?? null;
    }
} catch (Throwable $e) {
    $holidayToday = null;
    $upcomingHolidays = [];
...
$holidayDaysUntil = static function (string $d): int {
...
<?php if ($holidayToday || $upcomingHolidays || $nextHoliday): ?>
<!-- ── Belize public holidays — live "what's coming up" strip ──────────────── -->
<section class="border-b border-slate-200 bg-slate-50">
    <div class="mx-auto max-w-6xl px-6 py-4">
        ...
        <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5">
            ...
            <span class="text-sm font-black tracking-tight text-slate-900"><?= htmlspecialchars($holidayToday['name']) ?></span>
            <span class="text-xs font-semibold text-slate-500">
                Belize &middot; <?= date('l, F j', strtotime($holidayToday['holiday_date'])) ?><?php
                if ((float)$holidayToday['pay_rate'] > 1):
                ?> &middot; <?= htmlspecialchars(PublicHolidays::rateLabel((float)$holidayToday['pay_rate'])) ?> pay for hours worked<?php endif; ?>
            </span>
        </div>
        <?php endif; ?>

        <?php if ($upcomingHolidays): ?>
        <div class="<?= $holidayToday ? 'mt-3 ' : '' ?>flex flex-wrap items-center gap-2">
            <span class="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">
                <?= $holidayToday ? 'Also coming up' : 'Upcoming holidays in Belize' ?>
            </span>
            <?php foreach ($upcomingHolidays as $h): $du = $holidayDaysUntil($h['holiday_date']); ?>
            <span class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-600 shadow-sm">
                <span class="font-black tracking-tight text-slate-900"><?= htmlspecialchars($h['name']) ?></span>
                <span class="text-slate-300">&middot;</span>
                <span class="font-semibold text-slate-500"><?= date('D, M j', strtotime($h['holiday_date'])) ?></span>
        ...
        <div class="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-xs font-semibold text-slate-500">
            ...
            <span class="font-black tracking-tight text-slate-900"><?= htmlspecialchars($nextHoliday['name']) ?></span>
            <span class="text-slate-300">&middot;</span>
            <span><?= date('l, F j', strtotime($nextHoliday['holiday_date'])) ?></span>
            <span class="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-black uppercase tracking-wide text-slate-500">In <?= $holidayDaysUntil($nextHoliday['holiday_date']) ?> days</span>
        </div>
        <?php endif; ?>
